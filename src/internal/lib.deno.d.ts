/**
 * This is the kv-related API surface that Deno exposes to the runtime. This
 * file is NOT a module, it's a "classic" script that exposes the Deno namespace
 * globally when included in a project.
 *
 * @file
 * @see https://github.com/denoland/deno/blob/v1.33.2/cli/tsc/dts/lib.deno.unstable.d.ts#L1519-L2073
 */
...
declare namespace Deno {
   ...
   * **UNSTABLE**: New API, yet to be vetted.
   *
   * A key to be persisted in a {@linkcode Deno.Kv}. A key is a sequence of
   * {@linkcode Deno.KvKeyPart}s.
   *
   ...
   *   string
   * - `number` is ordered following this pattern: `-NaN` < `-Infinity` < `-100.0`
   *   < `-1.0` < -`0.5` < `-0.0` < `0.0` < `0.5` < `1.0` < `100.0` < `Infinity`
   *   < `NaN`
   * - `bigint` is ordered by mathematical ordering, with the largest negative
   *   number being the least first value, and the largest positive number being
   *   the last value
   ...
   * **UNSTABLE**: New API, yet to be vetted.
   *
   * A mutation to a key in a {@linkcode Deno.Kv}. A mutation is a combination of
   * a key, a value, and a type. The type determines how the mutation is applied
   * to the key.
   *
   ...
   *   the given value. Both the value specified in the mutation, and any
   ...
   *
   * This is the same as a {@linkcode KvEntry}, but the `value` and
   * `versionstamp` fields may be `null` if no value exists for the given key in
   * the KV store.
   *
   * @category KV
   */
   ...
   * **UNSTABLE**: New API, yet to be vetted.
   *
   * An operation on a {@linkcode Deno.Kv} that can be performed atomically.
   * Atomic operations do not auto-commit, and must be committed explicitly by
  ...
  export class AtomicOperation {
     ...
     * {@linkcode Deno.KvU64}, so the value of `n` must be in the range `[0,
     * 2^64-1]`.
     */
     ...
     * return value will be a {@linkcode Deno.KvCommitResult} object with a `ok:
   ...
   * value set for a given key is the one that is returned when reading the key.
   * Keys can be deleted from the database, in which case they will no longer be
   * returned when reading keys.
   *
  ...
  export class Kv {
     ...
     * in the form of a {@linkcode Deno.KvEntryMaybe}. If no value exists for the
     ...
     * ```
     *
     * The `consistency` option can be used to specify the consistency level for
     * the read operation. The default consistency level is "strong". Some use
     * cases can benefit from using a weaker consistency level. For more
     ...
     *   For example, the selector `["users", "a"], ["users", "n"]` will select
     *   all keys that start with the prefix `["users"]` and have a second key
     *   part that is lexicographically between `a` and `n`, such as `["users",
     *   "alice"]`, `["users", "bob"]`, and `["users", "mike"]`, but not
     *   `["users", "noa"]` or `["users", "zoe"]`.
     *
     * ```ts
     * const db = await Deno.openKv();
     ...
     * operations to complete before closing the underlying database
     * connection.
     */
    close(): Promise<void>;
  }
