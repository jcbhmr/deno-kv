...
type str = string;
type Path = string;
type PhantomData<T> = any;
const PhantomData = undefined;
...
const STATEMENT_KV_POINT_DELETE = "delete from kv where k = ?";
...
interface SqliteDbHandler<P extends SqliteDbHandlerPermissions> {
  default_storage_dir: Option<PathBuf>;
  _permissions: PhantomData<P>;
}

interface SqliteDbHandlerPermissions {
  check_read(self: object, p: Path, api_name: str): Result<void, AnyError>;
  check_write(self: object, p: Path, api_name: str): Result<void, AnyError>;
}

function SqliteDbHandler(default_storage_dir: Option<PathBuf>) {
  Object.assign(this, {
    default_storage_dir,
...
type DB = SqliteDb;
Object.assign(SqliteDbHandler.prototype, {
  async  open<P extends SqliteDbHandlerPermissions>(
    this: SqliteDbHandler<P>,
