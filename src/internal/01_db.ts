/**
 * This is adapted from the Deno source code for the kv extension. We needed to:
 *
 * 1. Replace all the globalThis.__bootstrap.primordials.* references with
 *    user-provided intrinsics. We use the getIntinsic() function from the
 *    get-intrinsic package to do this.
 * 2. Supply our own custom Deno.* types for the Deno namespace. This is done via
 *    the lib.deno.d.ts which is also from the Deno source code. We used just
 *    the parts that we needed for the kv extension.
 * 3. Properly prepare 'core' and 'ops' which are the two main objects that this
 *    module uses from the Deno.* namespace to perform operations. To do this,
 *    we can create a custom object that uses TS-defined functions for the op_*
 *    functions instead of Rust-defined functions. See the other .ts files in
 *    this directory for more information.
 *
 * @file
 * @see https://github.com/denoland/deno/blob/v1.33.2/ext/kv/01_db.ts
 */
...
const BigIntPrototypeToString = getIntrinsic("%BigInt.prototype.toString%");
const ObjectFreeze = getIntrinsic("%Object.freeze%");
const ObjectGetPrototypeOf = getIntrinsic("%Object.getPrototypeOf%");
const ObjectPrototypeIsPrototypeOf = getIntrinsic(
  "%Object.prototype.isPrototypeOf%"
);
const StringPrototypeReplace = getIntrinsic("%String.prototype.replace%");
const SymbolFor = getIntrinsic("%Symbol.for%");
const SymbolToStringTag = getIntrinsic("%Symbol.toStringTag%");
const Uint8ArrayPrototype = getIntrinsic("%Uint8Array.prototype%");
...
class Kv {
  ...
  async getMany(
    ...
    const ranges: RawKvEntry[][] = await core.opAsync(
      "op_kv_snapshot_read",
      this.#rid,
      keys.map((key) => [null, key, null, 1, false, null]),
      opts?.consistency ?? "strong"
    );
  ...
  async delete(key: Deno.KvKey) {
    const checks: Deno.AtomicCheck[] = [];
    const mutations = [[key, "delete", null]];
